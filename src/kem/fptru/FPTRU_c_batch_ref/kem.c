...
int crypto_kem_keygen_fixedbatch(unsigned char *pk, unsigned char *sk)
{
    unsigned int i,j;
    ...
    crypto_pke_keygen_batch(pk, sk, coins);
    for(j = 0; j < N_KEYBATCH; ++j){
      for (i = 0; i < FPTRU_PKE_PUBLICKEYBYTES; ++i){
        sk[i + FPTRU_PKE_SECRETKEYBYTES + j*FPTRU_KEM_SECRETKEYBYTES] = pk[i + j*FPTRU_KEM_PUBLICKEYBYTES];
      }
      randombytes(sk + FPTRU_PKE_SECRETKEYBYTES + FPTRU_PKE_PUBLICKEYBYTES + j*FPTRU_KEM_SECRETKEYBYTES, FPTRU_SEEDBYTES);
    }
  return 0;
}
